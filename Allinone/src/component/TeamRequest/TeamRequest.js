...
export default function TeamRequest() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch('http://54.84.53.208:5011/manager/140894/requests');
  ...
  const handleStatusChange = useCallback(async (currentRequestId, currentStatus) => {
    ...
    try {
      const response = await fetch(
        `http://54.84.53.208:5010/manage_request`,
        {
          method: "PUT",
          headers: {
  ...
  const handleWithdrawChange = useCallback(async (requestId) => {
    ...
    try {
      const response = await fetch(
        `http://54.84.53.208:5010/revoke_arrangments_by_request/${requestId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
