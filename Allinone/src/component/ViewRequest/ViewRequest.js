...
export default function ViewRequest() {
    ...
    const handleWithdrawArrangement = useCallback(async (arrangementId) => {
        ...
        try {
            const response = await fetch(`http://54.84.53.208:5005/withdraw_arrangement/${uid}/${arrangementId}`, {
                method: 'DELETE',
            });
            ...
            if (response.ok) {
                ...
                const statusResponse = await fetch(`http://54.84.53.208:5003/update_request/${uid}`, {
                    method: 'PATCH',
    ...
    useEffect(() => {
        const fetchRequestData = async () => {
            try {
                const response = await fetch(`http://54.84.53.208:5011/view_request/${uid}`);
                if (response.ok) {
        ...
        async function fetchArrangementDates() {
            try {
                const response = await fetch(
                    `http://54.84.53.208:5005/get_arrangements/request/${uid}`
                );
                if (!response.ok) {
