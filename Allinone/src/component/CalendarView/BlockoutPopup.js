...
const BlockoutPopup = () => {
  ...
  const handleInputChange = (field, value) => {
    ...
    switch (field) {
      ...
      case 'startDate':
        setStartDate(value);
        if (value && new Date(value) >= today && updatedErrors.includes('Date is required.')) {
          updatedErrors = updatedErrors.filter(error => error !== 'Date is required.');
        }

        if (value && updatedErrors.includes("The selected date must be a current or future date.")) {
